...
function validateCollection(dataset, name, idGetter) {
  ...
  for (const [index, record] of results.entries()) {
    ...
    if (!id) {
      throw new Error(
        `${name}: record ${index} has no stable ID`
      );
    }

    if (ids.has(id)) {
      throw new Error(
        `${name}: duplicate ID detected: ${id}`
      );
    }
...
function getCorruptionReasons(record) {
  ...
  if (
    ...
    reasons.push(
      'super_built_up_area_less_than_carpet_area'
    );
  }
...
function analyzeFakeCandidates(listings) {
  ...
  for (const listing of listings) {
    ...
    const names =
      suspiciousContacts.get(contact);
    ...
    candidates.push({
      ...
      other_names: names.filter(
        name =>
          name !== listing.posted_by_name
      ),
      reason:
        'same_contact_associated_with_multiple_names',
    });
  }

  return {
    suspiciousContacts:
      [...suspiciousContacts.entries()]
        .map(([contact, names]) => ({
          contact,
          names,
        })),
...
function buildFindings({
  ...
  for (const [endpoint, collection] of collections) {
    if (
      ...
      collection.reportedTotal !==
        collection.actualTotal
    ) {
      findings.push({
        endpoint,
        ...
        documented:
          'The collection total represents the available records.',
  ...
  findings.push({
    ...
    documented:
      'The login response documentation describes a token field.',

    actual:
      'The running API returns access_token and refresh_token fields.',

    how_found:
      'Verified the live login response used by the fetcher.',
  ...
  findings.push({
    ...
    how_found:
      'Verified the detail route against the running API.',

    impact:
      'Using the wrong route causes listing detail requests to fail.',
  ...
  findings.push({
    ...
    documented:
      'Analytics summary is documented as an available endpoint.',
  ...
  if (propertyAnalysis.duplicate_groups.length > 0) {
    findings.push({
      ...
      evidence:
        propertyAnalysis.duplicate_groups
          .slice(0, 20)
  ...
  if (q10.length > 0) {
    findings.push({
      ...
      impact:
        'Project inventory counts shown in the UI can be incorrect.',
...
async function main() {
  ...
  const listings =
    listingsCollection.records;

  const rentals =
    rentalsCollection.records;

  const projects =
    projectsCollection.records;

  console.log('\n─── Dataset validation ───');

  console.log(
    `Listings : ${listings.length} unique`
  );

  console.log(
    `Rentals  : ${rentals.length} unique`
  );

  console.log(
    `Projects : ${projects.length} unique`
  );
  ...
  const propertyAnalysis =
    analyzeUniqueProperties(listings);

  const q2 =
    propertyAnalysis.unique_count;
  ...
  const golfCourseRentals =
    rentals.filter(
      rental =>
        normalize(rental.locality) ===
        'golf course road'
    );

  const q5 =
    golfCourseRentals.reduce(
      (sum, rental) =>
        sum +
  ...
  const q8 =
    postedInSevenDays.length;

  /* Q9 */

  const fakeAnalysis =
    analyzeFakeCandidates(
      listings
    );
  ...
  for (const project of projects) {
    ...
    if (
      ...
      q10.push({
        ...
        reported_total_listings:
          reported,

        actual_listing_count:
          actual,
  ...
  console.log(
    `Q1 Total listing records : ${q1}`
  );

  console.log(
    `Q2 Unique properties     : ${q2}`
  );
  ...
  console.log(
    `Q3 Active listings       : ${q3}`
  );

  console.log(
    `Q4 Corrupt listings      : ${q4}`
  );

  console.log(
    `Q5 Monthly Golf Course Road rent : ` +
    `${formatINR(q5)}`
  );

  console.log(
    `Q6 Mean live 2BHK price/sqft : ₹${q6}`
  );
  ...
  if (costliestProject) {
    ...
    console.log(
      'Q7 Costliest project : unavailable'
    );
  }
  ...
  console.log(
    `Q9 Confirmed fake listings : ` +
    `${q9.confirmed_fake_listing_ids.length}`
  );

  console.log(
    `Q9 Suspicious candidates : ` +
    `${q9.suspicious_candidates.length}`
  );

  console.log(
    `Q10 Incorrect project listing counts : ` +
    `${q10.length}`
  );
  ...
  const output = {
    ...
    dataset: {
      listings: {
        ...
        actual_total:
          listingsCollection.actualTotal,
      },
      ...
      projects: {
        ...
        actual_total:
          projectsCollection.actualTotal,
      },
    },

    answers: {
      q1_total_listing_records:
        q1,
      ...
      q7_costliest_project: {
        ...
        unit:
          'Raw project price_max units as supplied by the dataset; no unsupported currency conversion is applied.',
      },
    ...
    methodology: {
      ...
      q2:
        'A property fingerprint is constructed from physical/property attributes. listing_id is not used as identity. Duplicate fingerprint groups are explicitly reported.',

      q4:
        'Only objectively impossible structural/numeric records are classified as corrupt.',

      q6:
        'Live listings, exactly 2 BHK, non-corrupt, positive price and positive carpet area. Price per sqft is calculated per listing before taking the arithmetic mean.',

      q7:
        'Projects are ranked by raw numeric price_max. No unsupported unit conversion is performed.',

      q8:
        'Seven-day half-open interval [2026-09-03T00:00:00+05:30, 2026-09-10T00:00:00+05:30). Bare listing timestamps are interpreted in Asia/Kolkata.',

      q9:
        'No weak heuristic is treated as proof of a fake listing.',

      q10:
        'Actual listing count is computed from the complete listings dataset grouped by project_id and compared with projects.total_listings.',
    },

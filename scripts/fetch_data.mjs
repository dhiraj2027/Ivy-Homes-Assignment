...
const EMAIL = String(
  process.env.IVY_EMAIL || ''
).trim();

const PASSWORD = String(
  process.env.IVY_PASSWORD || ''
);
...
function parseCollectionResponse(
  ...
  if (
    ...
    throw new Error(
      `${endpoint}: expected an object response`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${endpoint}: response.results is not an array`
    );
  }
  ...
  if (
    ...
    throw new Error(
      `${endpoint}: invalid response.total`
    );
  }
    ...
    try {
      ...
      if (text) {
        try {
          ...
          if (response.ok) {
            throw new Error(
              `${path}: API returned invalid JSON`
            );
          }
        }
      }
...
async function fetchAll(
  ...
  while (true) {
    ...
    if (
      ...
      console.log(
        `  API reported total : ${reportedTotal}`
      );
    ...
    console.log(
      `  Offset ${String(offset).padStart(4)} : ` +
      `${page.length} records`
    );

    for (
      const record of page
    ) {
      const id =
        recordIdentity(
          record
      ...
      if (
        ...
        throw new Error(
          `${endpoint}: duplicate ID "${id}" encountered`
        );
      }
  ...
  if (
    ...
    throw new Error(
      `${endpoint}: uniqueness validation failed`
    );
  }
  ...
  if (
    ...
    if (
      ...
      throw new Error(
        `${endpoint}: inconsistent empty collection state`
      );
    }
  }
  ...
  if (totalMatches) {
    console.log(
      `  ✓ API total matches actual count`
    );
  } else {
    console.warn(
      `  ⚠ API total mismatch: ` +
  ...
  console.log(
    `  ✓ Complete collection : ${actualTotal} records`
  );
...
async function fetchHealth() {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        '/health returned an invalid response'
      );
    }
...
function buildMetadata({
  ...
  return {
    ...
    pagination: {
      ...
      note:
        'API-reported totals are retained as metadata and are not used as the pagination termination condition.',
    },

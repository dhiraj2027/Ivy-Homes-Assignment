...
const VALID_SORTS = new Set(["posted_at", "price", "carpet_area"]);
...
function matchesFilters(listing, filters) {
  const requestedLocality = normalizeText(filters.locality);

  if (
    requestedLocality &&
    normalizeText(listing.locality) !== requestedLocality
  ) {
    return false;
  }

  const requestedBhk = normalizeText(filters.bhk);

  const bedroom = toNumber(listing.bedroom);

  if (requestedBhk === "5+") {
    if (bedroom === null || bedroom < 5) {
      return false;
    }
  } else if (requestedBhk) {
    if (bedroom === null || bedroom !== Number(requestedBhk)) {
      return false;
    }
  }

  const requestedMinPrice = normalizeText(filters.min_price);
  ...
  if (requestedMinPrice) {
    ...
    if (!Number.isFinite(minPrice) || price === null || price < minPrice) {
      return false;
    }
  }

  const requestedMaxPrice = normalizeText(filters.max_price);

  if (requestedMaxPrice) {
    const maxPrice = Number(requestedMaxPrice);
  ...
  const requestedFurnishing = normalizeText(filters.furnishing);

  if (
    requestedFurnishing &&
    normalizeText(listing.furnishing) !== requestedFurnishing
  ) {
    return false;
  }

  const requestedPropertyType = normalizeText(filters.property_type);

  if (
    requestedPropertyType &&
    normalizeText(listing.property_type) !== requestedPropertyType
  ) {
    return false;
  }
...
function compareValues(first, second, sortBy) {
  if (sortBy === "posted_at") {
    const firstTime = Date.parse(first?.posted_at);

    const secondTime = Date.parse(second?.posted_at);
  ...
  const field = sortBy === "price" ? "price" : "carpet_area";

  const firstValue = toNumber(first?.[field]);

  const secondValue = toNumber(second?.[field]);
...
export default function Listings() {
  ...
  const [view, setView] = useState(() => {
    try {
      const stored = localStorage.getItem("ivy_listings_view");

      return stored === "list" ? "list" : DEFAULT_VIEW;
    } catch {
      return DEFAULT_VIEW;
    }
  });
  ...
  const filters = useMemo(() => {
    const requestedSort = normalizeString(searchParams.get("sort_by"));

    const requestedOrder = normalizeString(searchParams.get("order"));

    return {
      page: toPositiveInt(searchParams.get("page"), 1),

      locality: searchParams.has("locality")
        ? normalizeString(searchParams.get("locality"))
        : DEFAULT_LOCALITY,

      bhk: normalizeString(searchParams.get("bhk")),

      min_price: normalizeString(searchParams.get("min_price")),

      max_price: normalizeString(searchParams.get("max_price")),

      furnishing: normalizeString(searchParams.get("furnishing")),

      property_type: normalizeString(searchParams.get("property_type")),

      sort_by: VALID_SORTS.has(requestedSort) ? requestedSort : DEFAULT_SORT,

      order: VALID_ORDERS.has(requestedOrder) ? requestedOrder : DEFAULT_ORDER,
    };
  }, [searchParams]);
  ...
  const updateFilters = useCallback(
    (updates = {}) => {
      ...
      Object.entries(merged).forEach(([key, value]) => {
        ...
        if (
          key === "locality" &&
          normalizeText(value) === normalizeText(DEFAULT_LOCALITY)
        ) {
          return;
        }

        if (key === "sort_by" && value === DEFAULT_SORT) {
          return;
        }

        if (key === "order" && value === DEFAULT_ORDER) {
          return;
        }
  ...
  const changeView = useCallback((nextView) => {
    ...
    try {
      localStorage.setItem("ivy_listings_view", nextView);
    } catch {
  ...
  const filteredListings = useMemo(() => {
    ...
    result.sort((first, second) => {
      const comparison = compareValues(first, second, filters.sort_by);
  ...
  useEffect(() => {
    if (!loading && filters.page !== safePage) {
      updateFilters({
        page: safePage,
      });
    }
  }, [loading, filters.page, safePage, updateFilters]);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy-900">Sale Listings</h1>
            ...
            <>
              ...
              >
                {paginatedListings.map((listing) => {
                  ...
                  return (
                    <ListingCard key={String(listingId)} listing={listing} />
                  );
                })}
              </div>

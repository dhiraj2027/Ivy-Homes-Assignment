...
import { getFavourites, removeFavourite } from "../api/client.js";
...
function FavCard({ listing, onRemove, removing }) {
  ...
  const apartmentName = normalizeText(listing?.apartment_name);

  const locality = normalizeText(listing?.locality);
  ...
  const liveStatus = normalizeBoolean(listing?.is_live);

  const isLive = liveStatus === true;
  const statusKnown = liveStatus !== null;

  const validPrice = isValidPositiveNumber(price);

  const validArea = isValidPositiveNumber(carpetArea);

  const validBedroom = isValidCount(bedroom);

  const validBathroom = isValidCount(bathroom);

  const listingPath = `/listings/${encodeURIComponent(listingId)}`;

  return (
    <article
      className={`rounded-xl border bg-white p-4 transition-all ${
        isLive ? "border-gray-200" : "border-gray-100 opacity-75"
      }`}
    >
      <div
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        >
          <p className="truncate text-sm font-semibold text-navy-900 hover:underline">
            {apartmentName || "Unnamed property"}
          </p>

          <p className="mt-0.5 truncate text-xs text-gray-500">
            {locality ? titleCase(locality) : "Locality unavailable"}
          </p>
        </Link>
      ...
      >
        ...
        <div className="my-3">
          <p className="tabular-nums text-xl font-bold text-navy-900">
            {validPrice ? formatPrice(price) : "Price unavailable"}
          </p>

          {validPrice && validArea && (
            <p className="text-xs text-gray-400">
              {formatPricePerSqft(price, carpetArea)}
            </p>
          )}
        </div>
        ...
        {(validBedroom ||
          ...
          <div className="mb-3 flex flex-wrap items-center gap-3 border-t border-gray-100 pt-3 text-xs text-gray-500">
            {validBedroom && (
              <span>
                {Number(bedroom)} {Number(bedroom) === 1 ? "bed" : "beds"}
              </span>
            )}

            {validBathroom && (
              <span>
                {Number(bathroom)} {Number(bathroom) === 1 ? "bath" : "baths"}
              </span>
            )}

            {validArea && <span>{formatArea(carpetArea)}</span>}

            {hasText(furnishing) && <Tag>{titleCase(furnishing)}</Tag>}
          </div>
        )}
        ...
        <div className="flex items-center justify-between gap-3">
          <span className="text-xs text-gray-400">
            {hasText(postedAt) ? formatRelative(postedAt) : "Date unavailable"}
          </span>
...
export default function Favourites() {
    ...
    async ({ signal, showLoading = true } = {}) => {
      ...
      try {
        ...
        if (!response || !Array.isArray(response.results)) {
          throw new Error("Invalid favourites response from the API.");
        }
  ...
  }, [isAuthenticated, refreshing, load]);

  const handleRemove = useCallback(
    async (listingId) => {
      const normalizedId = normalizeText(listingId);

      if (!normalizedId || removingId !== null) {
        return;
      }
      ...
      try {
        ...
        setFavourites((current) =>
          current.filter(
            (listing) => normalizeText(listing?.listing_id) !== normalizedId
          )
        );
      } catch (err) {
        setError(err?.message || "Could not remove the saved listing.");
      } finally {
        setRemovingId(null);
      }
    },
    ...
    return (
      <main className="mx-auto max-w-2xl px-4 py-16 text-center">
        ...
        <p className="mt-2 mb-6 text-sm text-gray-500">
          Your saved properties are tied to your account.
        </p>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {favourites.map((listing) => {
            const id = normalizeText(listing?.listing_id);

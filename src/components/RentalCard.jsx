...
export default function RentalCard({ rental }) {
  const listingId = normalizeText(rental?.listing_id)
  ...
  const validRent = isValidPositiveNumber(price)

  const validDeposit = isValidNonNegativeNumber(deposit)

  const validMaintenance = isValidNonNegativeNumber(maintenance)
  ...
  const listingPath = `/rentals/${encodeURIComponent(listingId)}`

  return (
    <Link
      to={listingPath}
      aria-label={`View rental ${apartmentName || "property"}`}
    ...
    >
      <div className="h-1 rounded-t-xl bg-blue-400" aria-hidden="true" />
      ...
      <div className="p-4">
        ...
        <div className="mb-2">
          ...
          <p className="mt-0.5 flex min-w-0 items-center gap-1 text-xs text-gray-500">
            ...
            {floorLabel && <span className="shrink-0">{floorLabel}</span>}
          </p>
        </div>
        ...
        <div className="my-3">
          <p className="tabular-nums text-2xl font-bold leading-none text-navy-900">
            {validRent ? formatRent(price) : "Rent unavailable"}
          </p>

          {(validDeposit || validMaintenance) && (
            <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-400">
              {validDeposit && <span>Deposit: {formatPrice(deposit)}</span>}

              {validMaintenance && (
                <span>Maint: {formatRent(maintenance)}</span>
              )}
            </div>
          )}
        </div>
        ...
        {(validBedroom || validBathroom || validArea) && (
          <div className="flex flex-wrap items-center gap-x-3 gap-y-2 border-t border-gray-100 pt-3 text-xs text-gray-600">
            ...
            {validBathroom && (
              <span className="flex items-center gap-1">
                <Bath size={12} className="text-gray-400" aria-hidden="true" />
        ...
        <div className="mt-3 flex flex-wrap items-end justify-between gap-2">
          <div className="flex flex-wrap gap-1">
            {hasText(propertyType) && <Tag>{titleCase(propertyType)}</Tag>}

            {hasText(furnishing) && <Tag>{titleCase(furnishing)}</Tag>}

            {hasText(postedBy) && <Tag>{titleCase(postedBy)}</Tag>}
          </div>

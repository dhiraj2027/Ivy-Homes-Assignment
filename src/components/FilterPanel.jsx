...
function RadioOption({ name, value, checked, onChange, children }) {
  return (
    <label className="flex cursor-pointer items-center gap-2 rounded py-0.5">
      ...
      <span className="text-sm capitalize text-gray-600">{children}</span>
    </label>
  );
}
...
export default function FilterPanel({
  ...
  const selectedBhk = normalizeText(filters.bhk);

  const selectedFurnishing = normalizeText(filters.furnishing);

  const selectedPropertyType = normalizeText(filters.property_type);

  const [localityInput, setLocalityInput] = useState(
    normalizeText(filters.locality) || DEFAULT_LOCALITY
  );

  const [minPriceInput, setMinPriceInput] = useState(
    normalizeText(filters.min_price)
  );

  const [maxPriceInput, setMaxPriceInput] = useState(
    normalizeText(filters.max_price)
  );

  useEffect(() => {
    setLocalityInput(normalizeText(filters.locality) || DEFAULT_LOCALITY);

    setMinPriceInput(normalizeText(filters.min_price));

    setMaxPriceInput(normalizeText(filters.max_price));
  }, [filters.locality, filters.min_price, filters.max_price]);
  ...
  const commitPrice = (minValue = minPriceInput, maxValue = maxPriceInput) => {
    const normalizedMin = normalizeText(minValue);

    const normalizedMax = normalizeText(maxValue);

    const numericMin = normalizedMin === "" ? null : Number(normalizedMin);

    const numericMax = normalizedMax === "" ? null : Number(normalizedMax);
    ...
    const currentMin = normalizeText(filters.min_price);

    const currentMax = normalizeText(filters.max_price);
  ...
  const clearLocality = () => {
    ...
    if (normalizeText(filters.locality)) {
      setFilter("locality", "");
    }
  };
  ...
  const clearFurnishing = () => {
    if (selectedFurnishing) {
      setFilter("furnishing", "");
    }
  };

  const clearPropertyType = () => {
    if (selectedPropertyType) {
      setFilter("property_type", "");
    }
  };
  ...
  const clearPrices = () => {
    ...
    if (normalizeText(filters.min_price) || normalizeText(filters.max_price)) {
      onChange({
        min_price: "",
        max_price: "",
        page: 1,
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {hasFilters && (
          <button
            type="button"
            onClick={() => {
              setLocalityInput(DEFAULT_LOCALITY);
              setMinPriceInput("");
              setMaxPriceInput("");
              onReset();
            }}
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {BHK_OPTIONS.map((bedrooms) => {
            const value = String(bedrooms);

            const active = selectedBhk === value;

            return (
              <Pill
                key={value}
                active={active}
                onClick={() => setFilter("bhk", active ? "" : value)}
              >
                {bedrooms} BHK
              </Pill>
            );
          ...
          <Pill
            active={selectedBhk === "5+"}
            onClick={() => setFilter("bhk", selectedBhk === "5+" ? "" : "5+")}
          >
            5+
          </Pill>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          {FURNISHING_OPTIONS.map((furnishing) => (
            <RadioOption
              ...
              name="furnishing"
              value={furnishing}
              checked={selectedFurnishing === furnishing}
              onChange={(value) => setFilter("furnishing", value)}
            >
              {formatOptionLabel(furnishing)}
            </RadioOption>
          ))}
        </div>
      </div>
        ...
        <div className="flex flex-col gap-1.5">
          {PROPERTY_TYPES.map((propertyType) => (
            <RadioOption
              ...
              onChange={(value) => setFilter("property_type", value)}
            >
              {propertyType}
            </RadioOption>
          ))}
      ...
      <div>
        ...
        <select
          value={`${normalizeText(
            filters.sort_by || "posted_at"
          )}:${normalizeText(filters.order || "desc")}`}
          onChange={(event) => {
        ...
        >
          ...
          <option value="carpet_area:asc">Area: Small to Large</option>
